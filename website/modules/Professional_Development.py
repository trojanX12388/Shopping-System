...
from website.models import FISFaculty, FISAdmin, FISProfessionalDevelopment
...
@PD.route("/Admin-Professional-Development", methods=['GET', 'POST'])
@login_required
@Check_Token
def adminPD_T():
    # INITIALIZING DATA FROM USER LOGGED IN ACCOUNT    
        ...
        if request.method == 'POST':
            ...
            nameFile = f'{str(username.AdminId)}{str(namefile)}'

            
        # PROFESSIONAL DEVELOPMENT FOLDER ID
            folder = '1PeOWI6_4q71BaC3TvqW9HIgGM2SHRCIF'
            ...
            try:
                ...
                flash('successfully updated!', category='success')
                return redirect(url_for('PD.adminPD_T'))
            
            except:
                flash('Unsuccessfully Updated... please try again.', category='error') 
                return redirect(url_for('PD.adminPD_T'))
                      
        return render_template("Admin-Home-Page/Professional-Development/Professional-Development.html", 
                               User= username.FirstName + " " + username.LastName,
                               faculty_code= username.FacultyCode,
                               user= current_user,
                               profile_pic=ProfilePic)
...
@PD.route("/Admin-Professional-Development/add-record", methods=['GET', 'POST'])
@login_required
def adminPD_add():
        ...
        id = f'{str(username.AdminId)}{str(namefile)}'
...
@PD.route("/Admin-Professional-Development/delete-record", methods=['GET', 'POST'])
@login_required
def adminPD_del():
        username = FISAdmin.query.filter_by(AdminId=current_user.AdminId).first() 
