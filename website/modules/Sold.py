...
@sold.route("/sold/packing", methods=['GET', 'POST'])
@login_required
@Check_Token
def FN_R():
   # UPDATE 
        ...
        if request.method == 'POST':
            ...
            u = delivering(MSNotification)
            u = u.values({"status": 'packing',"Status": 'packing',})
            
            u = u.where(MSNotification.id == id)
            
            db.session.execute(u)
            db.session.commit()
            db.session.close()
            
            return redirect(url_for('sold.S_H'))
        


@sold.route("/sold/unread", methods=['GET', 'POST'])
@login_required
@Check_Token
def FN_UR():
   # UPDATE 
        ...
        if request.method == 'POST':
            ...
            u = delivering(MSNotification)
            u = u.values({"status": 'notif',"Status": 'packing',})
...
@sold.route("/sold/trash", methods=['GET', 'POST'])
@login_required
@Check_Token
def FN_T():
    # UPDATE 
        ...
        if request.method == 'POST':
            ...
            u = delivering(MSNotification)
            u = u.values({"status": 'trash', "Status": 'trash',})
...
@sold.route("/sold/delete", methods=['GET', 'POST'])
@login_required
@Check_Token
def FN_D():
    # DELETE RECORD
        ...
        data = MSNotification.query.filter_by(id=id).first() 
